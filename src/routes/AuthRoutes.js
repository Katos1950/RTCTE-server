...
function generateAccessToken(user) {
    return jwt.sign({ emailId: user.emailId }, process.env.ACCESS_TOKEN_SECRET, { expiresIn: "2h" });
}
...
router.post("/login", async (req, res) => {
    try {
        ...
        const refreshToken = jwt.sign({ emailId: user.emailId }, process.env.REFRESH_TOKEN_SECRET, { expiresIn: "7d" });
